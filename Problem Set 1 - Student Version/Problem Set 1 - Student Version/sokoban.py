...
class SokobanState:
    ...
    def are_facing_each_other(self, crate1: Point, crate2: Point) -> bool:
        dx = crate2.x - crate1.x
        dy = crate2.y - crate1.y
        return abs(dx) == 1 and dx == -dy or abs(dy) == 1 and dy == -dx
...
class SokobanProblem(Problem[SokobanState, Direction]):
    ...
    def modifiedgetactions(self, state: SokobanState) -> Iterable[Direction]:
        actions = []
        for direction in Direction:
            position = state.player + direction.to_vector()
            # Disallow walking into walls
    ...
    def modifiedgetsuccessor(self, state: SokobanState, action: Direction) -> SokobanState:
        player = state.player + action.to_vector()
        crates = state.crates
        if player not in self.layout.walkable:
            # If we try to walk into a wall, then this action is wrong
    ...
    def from_text(text: str) -> 'SokobanProblem':
        ...
        problem.layout = SokobanLayout(
            width, height, frozenset(walkable), frozenset(goals))
        problem.initial_state = SokobanState(
            problem.layout, player, frozenset(crates))
        return problem
